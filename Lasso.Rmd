...
```{r}
library(glmnet)
x = model.matrix(price ~ . -sqft_living + I((sqft_living)) -sqft_lot + I((sqft_lot)), data = seattle.df)[,-1]
y = seattle.df$price
```
...
The root mean squared error is 146390.98, which is slightly lower than the error from the multiple linear regression model (149852.57). 
