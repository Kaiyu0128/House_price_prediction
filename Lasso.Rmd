...
```{r}
library(glmnet)
x = model.matrix(price ~ . -sqft_lot + I((sqft_lot)) -sqft_lot + I((sqft_lot)), data = seattle.df)[,-1]
y = seattle.df$price
```
...
The best lambda found with 10-fold cross-validation is 248. After performing the lasso, we find that none of the coefficients of the predictors were shrunk to zero. yr_renovated and log(sqft_lot) are relatively close to 0 compared to the others, with coefficients of 3.6 and -5.9 respectively. 
...
The root mean squared error is 146390.65, which is slightly higher than the error from OLS (146234.26). 
...
The lasso model R2 = 0.51, so a little more than half of the variation in price is explained by the linear relationship with the predictors. This is a slight improvement from the multiple linear regression model R2 = 0.485.
