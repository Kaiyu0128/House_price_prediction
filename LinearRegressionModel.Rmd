---
title: "House Price Prediction Model 1: Linear Regression"
output:
...
sqft_lot has a strong right skew, so we will consider using a log-transformation on sqft_lot.
...
Now recall from the analysis of the individual scatterplots of price against the predictors that sqft_lot, yr_built, and yr_renovated appear to have no linear relationship with price. We apply a log-transformation to these predictors.
...
plot(price ~ I(log(yr_renovated+0.5)), data = seattle.df, main="Price vs log(yr_renovated+0.5)", lwd=2)
  abline(lm(price~I(log(yr_renovated+0.5)), data=seattle.df),col="red",lwd=2)
```
...
seattle.lm3 = update(seattle_full.lm, ~ . -yr_renovated - sqft_lot + I(log(sqft_lot)))
```
...
```
### Conclusion
Our linear regression model of price resulted in an R^2 of 0.4854, meaning that almost half of the variability in price is explained by a linear relationship with the predictors. The root mean squared error is 146234.26, which means our model's estimate of housing prices is off by that much on average. 
