...
#### Model 2: Selection based on p-value
```{r}
...
Now we select a model using cross-validation.

#### Model 4: Selection using cross-validation
...
seattle.glm1 = glm(price ~ ., data = seattle.df, subset=train)
seattle.glm2 = glm(price ~ . - yr_renovated, data = seattle.df,subset=train)

cv.error = cv.glm(seattle.df[train,],seattle.glm1,K=10)$delta[1]
cv.error2 = cv.glm(seattle.df[train,],seattle.glm2,K=10)$delta[1]
```
