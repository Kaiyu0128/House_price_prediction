...
Assumptions for linear regression: 1) little to no multicollinearity, 2) linear relationship between predictors and response, 3) normally-distributed variables, 4) normally-distributed residuals
...
```{r}
#Checking linear relationship between response and predictors
#May need to transform predictors (e.g. log-transform)
...
There is nothing unusual with the residuals. We can assume the variables are normally distributed. 
...
ROUGH DRAFT: 
Summary. Our linear regression model of price resulted in an R^2 of 0.4906, meaning that almost half of the variability in price is explained by a linear relationship with the predictors. We have a large mean square error = 22455792622, which we want to reduce. There is too much variance in the model. Next, we will use the lasso, which will hopefully improve on the linear regression model. 
